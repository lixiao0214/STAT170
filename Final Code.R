...
ts.plot(ADoutofsample,forecast_es,lty=1:2,col=c("black","red"), ylab="weeks",
        main = "Forecast Plot")
legend("topright",legend= c("Actual","Forecast"),lty = c(1,2),col=c("black","red"))
...
total <- c()
for(i in 1:12){
  total[i] <- (exp(Y.pred$pred)[i]+exp(Y.pred2)[i]+exp(hat.x.ts)[i]+forecast_es[i])/4
}
total
...
ts.plot(ADoutofsample,exp(hat.x.ts),lty=1,col=c("black","green"), ylab="weeks",
        main = "Forecast Plot")
lines(forecast_es,col="red")
lines(ADoutofsample,lty=1,col="black")
lines(exp(Y.pred$pred),col="blue")
lines(exp(Y.pred2),col="dark grey")
legend("bottomleft",legend = c("Actual","ARIMA","VAR","Regression","Exponential Smoothing"),
